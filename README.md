...
| 模块 | 关键特性 | 对应源文件 |
| :--- | :--- | :--- |
| **Zsh** | Powerlevel10k 即时提示、zoxide 极速目录跳转 (保留 `j`/`ji`)、fnm (Node.js)、uv 补全、智能代理 `tproxy` / `tunproxy`、原生 Git URL 提取 `gurl`、fd 极速文件检索 `ff`、历史记录防泄密、本地逃生舱 `~/.zshrc.local` | `dot_zshrc.tmpl` |
...
| **Vim** | 精简原生 ~115 行、单文件自洽 (内置官方旗舰配色 habamax，兼顾 desert)、自愈式跨会话持久撤销 (`undofile`)、双拼秒退编辑态 (`kj`)、代码折叠与无缝窗格跳转 | `dot_vimrc` |
